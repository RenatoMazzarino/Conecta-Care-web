...
import type { Patient, Professional, Shift } from '@/lib/types';
...
import { Check, Filter, MessageSquare, Shield, Star, X } from 'lucide-react';
...
const mockFallbackContext: { shift: Shift; patient: Patient } = {
  shift: {
    id: 'shift-fallback',
...
export type CandidacyContext = { shift: Shift; patient: Patient };

type CandidacyDialogProps = {
  isOpen: boolean;
...
export function CandidacyManagementDialog({
  ...
  const activeContext = context ?? mockFallbackContext;
  const formattedDate = activeContext.shift.dayKey
    ? new Date(activeContext.shift.dayKey).toLocaleDateString('pt-BR', {
        timeZone: 'UTC',
        day: '2-digit',
        month: 'long',
      })
  ...
  const sortedAndFilteredCandidates = React.useMemo(() => {
    return candidates
      ...
      .sort((a, b) => {
        ...
        const aHasWorked = a.compatibilityTags?.some((t) =>
          t.text.includes('Já atendeu')
        );
        const bHasWorked = b.compatibilityTags?.some((t) =>
          t.text.includes('Já atendeu')
        );
  ...
  }, [candidates, minRating]);

  const handleApprove = (professional: Professional) => {
    onApprove(professional, activeContext.shift);
    toast({
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Revise, filtre e aprove o profissional ideal para o plantão de{' '}
            <strong>{activeContext.patient.name}</strong> em {formattedDate} (
            {activeContext.shift.shiftType}).
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-[280px_1fr] gap-6 flex-1 min-h-0">
          ...
          <div className="flex flex-col min-h-0">
            <p className="text-sm text-muted-foreground mb-2">
              {sortedAndFilteredCandidates.length} de {candidates.length}{' '}
              profissionais exibidos.
            </p>
                ...
                {sortedAndFilteredCandidates.map((candidate) => (
                  <div
                    ...
                    className={cn(
                      ...
                      candidate.corenStatus !== 'active' && 'opacity-60 bg-muted'
                    )}
                  >
                    <div className="flex items-start justify-between">
                      ...
                      >
                        <Avatar className="h-12 w-12">
                          <AvatarImage
                            ...
                            alt={candidate.name}
                            data-ai-hint={candidate.avatarHint}
                          />
                      ...
                      >
                        ...
                        <Button
                          ...
                          disabled={candidate.corenStatus !== 'active'}
                        >
