...
import { User, Phone, Mail, Calendar, Home, Link as LinkIcon, Gavel, BadgeCheck } from 'lucide-react';
...
const FormField = ({ label, value, action, isFullWidth }: { label: string, value: React.ReactNode, action?: React.ReactNode, isFullWidth?: boolean }) => (
    <div className={cn(isFullWidth && "md:col-span-2")}>
        <Label className="flex items-center gap-2 text-muted-foreground">{label}{action}</Label>
              ...
              <FormField
                ...
                action={
                  <Badge variant={displayData.cpfStatus === 'valid' ? 'secondary' : 'destructive'} className={cn('ml-2', displayData.cpfStatus === 'valid' && 'bg-green-100 text-green-800')}>
                    <BadgeCheck className="h-3 w-3 mr-1" />{displayData.cpfStatus === 'valid' ? 'Verificado' : 'Pendente'}
                  </Badge>
                }
              />
              <FormField
