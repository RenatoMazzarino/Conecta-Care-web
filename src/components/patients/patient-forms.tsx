...
export function PatientAddressForm({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback({ type: "error", message: error instanceof Error ? error.message : "Erro ao salvar endereço." });
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="border rounded-lg p-4 space-y-4 bg-white shadow-sm">
      <h3 className="text-lg font-semibold">Atualizar endereço e ambiente</h3>
      <form onSubmit={handleSubmit} className="space-y-4">
...
export function PatientClinicalSummaryForm({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback({ type: "success", message: "Resumo clínico salvo!" });
      onSaved?.();
    } catch (error: unknown) {
      const message =
        error instanceof SyntaxError
          ? "JSON inválido. Verifique o formato do resumo/meta."
          : error instanceof Error
          ? error.message
          : "Erro ao salvar resumo clínico.";
      setFeedback({ type: "error", message });
    } finally {
      setPending(false);
    }
  ...
  return (
    <div className="border rounded-lg p-4 space-y-4 bg-white shadow-sm">
      <h3 className="text-lg font-semibold">Atualizar resumo clínico</h3>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1">
          <label className="block text-sm font-medium text-slate-700">
            Resumo (JSON)
    ...
    try {
      ...
      if (invoiceHistoryText.trim()) {
        try {
          ...
          throw new Error("Hist�rico de faturas precisa ser um JSON valido.");
        }
      } else {
